...
def _create_xaxis_date(ax, date_index):
...
def line_plot(ts, ax=None, legend=True, labels=None, ylabel="Value", **kwargs):
    ...
    _create_xaxis_date(ax, date_index)
    ax.set_title(ts._generate_title(), fontsize=18)

    ax.plot(date_index, ts.data, **kwargs)
    ax.set_ylabel(ylabel)
    if legend:
        if not labels:
            labels = ts._generate_labels()
        if labels != [""]:
            ax.legend(title="Streams", labels=labels[:5])


# rename to dist plot
def dist_plot(ts, ax=None, percentiles=None, xlabel="Value", **kwargs):
    """ 
    ...
    ax.set_title("Distribution for " + ts._generate_title(), fontsize=18)
    ax.set_ylabel("Count")
    ax.set_xlabel(xlabel)
    if percentiles:
        m = max(hist[0])
...
def stacked_plot(ts, ax=None, ylabel="Value", **kwargs):
...
def timebox_plot(ts, ax=None, plot="auto", ylabel="Value", **kwargs):
    """
...
def correlogram_plot(ts, ax=None, **kwargs):
    """Plot stacked charts using sane time series defaults with Matplotlib.
...
def text_plot(value, ax=None, title="", fontsize=48, thresholds=None, **kwargs):
    """ 
    Plot a single value
...
def heatmap_plot(ts, ax=None, **kwargs):
    raise NotImplementedError

def pacf(ts, ax=None, **kwargs):
    raise NotImplementedError


def acf(ts, ax=None, **kwargs):
    raise NotImplementedError


def qq_plot(ts, ax=None, **kwargs):
    raise NotImplementedError


def probability_plot(ts, ax=None, **kwargs):
    raise NotImplementedError


def polar_plot(ts, ax=None, **kwargs):
    raise NotImplementedError
